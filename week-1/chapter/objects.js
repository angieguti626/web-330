...
let pokerGame = {
  ...
  payBet: function (type) {
    ...
    pokerHand.prototype.getHandValue = function () {
      ...
      | the type of hand held by a 5-card poker hand.    |
      +-------------------------------------------------*/

      function handType(pokerHand) {
        /* Determine the rank va,ue of each card in the hand
           by creating a property named rankValue         */
        for (let i = 0; i < pokerHand.cards.lenth; i++) {
          if (pokerHand.cards[i].rank === "ace") {
            pokerHand.cards[i].rankValue = 14;
          }
        }

        // Return a text string describing the hand for the draw poker //
